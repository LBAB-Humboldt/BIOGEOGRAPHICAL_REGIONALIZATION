...
#MASCARAS
colombia=readShapePoly("C:/Users/GIC 9/Documents/PARAMOS//SELECCION2/VACIOS/INFO_GEO/COLOMBIA.shp")
paramos=colombia
...
####Cambiar extent
# modelo=raster(nombres[[1]])
# modelo[which(modelo[]>1)]=NA
# modelo=mask(modelo,mascara)
# plot(modelo)
# extent=extent(modelo)
# aoi_ext=crop(aoi,extent)
# extent(aoi_ext)=extent(modelo)
...
#   especie[which(especie[]>1)]=NA
#   especie[which(especie[]<0)]=NA
#   #especie=aggregate(especie,factor,fun=min)
#   #capa=crop(especie,extent)
#   extent(especie)<-extent(modelo)
#   #corte=mask(especie,mascara)
#   corte=aggregate(especie,factor,fun=max)
#   muestreo=resample(corte,aoi_ext)
#   MODELOS=addLayer(MODELOS,muestreo)
# }
...
vacias=c(which(rowMeans(DF[,1:ncol(DF)])==0)) # solo estamos limpiando vacias , se puede menos de 5 (kreft)
